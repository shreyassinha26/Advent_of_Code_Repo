...
long long dp(vector<vector<long long>> &arr, vector<string> &block, int curr,
             std::size_t line, int DEPTH = 0) {
  if (line >= block.size()) {
    // cout<<DEPTH<<"\n";
    return 1;
  }
...
signed main(/*int argc, char **argv*/) {
  ...
  vector<vector<long long>> arr(block.size(),
                                vector<long long>(block.front().size(), 0));
