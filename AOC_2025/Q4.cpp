...
int main()
{
    ...
    if(block[0][0]=='@')count++;
    if(block[m-1][0]=='@')count++;
    if(block[0][n-1]=='@')count++;
    if(block[m-1][n-1]=='@')count++;
    block[0][0]='.';
    block[m-1][0]='.';
    block[0][n-1]='.';
    block[m-1][n-1]='.';
    for(auto &i : block){for(auto &j : i)if(j=='x')j='.';}
    return count;
    };
    int curr = iter();
    totalcount = curr;
    while(curr){curr = iter();totalcount += curr;}
    cout<< totalcount<<"\n";
